...
const aoblmoaClarificationTarget = "aoblmoa"

//go:embed docs/reference-data/aoblmoa-2023-clarification-request.json
var aoblmoa2023ClarificationJSON []byte

func TestAOBLMOA2023ClarificationRequestPinsEvidenceGates(t *testing.T) {
	t.Parallel()
	...
	err := json.Unmarshal(aoblmoa2023ClarificationJSON, &clarification)
	if err != nil {
		t.Fatalf("decode AOBLMOA clarification request: %v", err)
	}

	if clarification.SchemaVersion != 1 ||
		clarification.ProtocolID != "aoblmoa-2023-tables5-23" ||
	...
	if contact.CorrespondingAuthor != "Changsheng Huang" ||
		!strings.HasSuffix(contact.PublicInstitutionalEmail, "@upc.edu.cn") ||
		contact.ContactSource == "" || contact.Affiliation == "" || contact.SelectionBasis == "" {
		t.Fatalf("clarification request has no auditable public contact path: %+v", contact)
	}

	wantExactIDs := []string{
		"strategy_and_branch_semantics",
		"aquila_mean_semantics",
		"levy_scale_semantics",
		"population_and_lifecycle",
		"evaluation_accounting",
	}
	wantComparisonIDs := []string{
		"benchmark_and_driver_archive",
		"cec2017_protocol",
		"cec2020_constrained_protocol",
		"seeds_raw_runs_and_statistics",
	...
	if clarification.ExactPresetGate.State != "blocked_missing_author_or_archival_data" ||
		clarification.ExactPresetGate.TargetAlgorithm != aoblmoaClarificationTarget ||
		clarification.ExactPresetGate.Rule == "" ||
		clarification.ReproductionComparisonGate.State !=
			"blocked_missing_historical_run_data_and_drivers" ||
		clarification.ReproductionComparisonGate.Rule == "" ||
		clarification.PublicSourceAudit.Outcome == "" ||
		len(clarification.PublicSourceAudit.Sources) < 8 {
		t.Fatalf("clarification does not preserve its evidence gates: %+v",
			clarification.ExactPresetGate)
	}
	...
	if clarification.CorrespondenceDraft.Status != "not_sent" ||
		clarification.CorrespondenceDraft.Subject == "" ||
		clarification.CorrespondenceDraft.Body == "" {
		t.Fatalf("clarification request is not send-ready: %+v",
			clarification.CorrespondenceDraft)
	}
}

func TestAOBLMOA2023ReferenceLinksClarificationRequest(t *testing.T) {
	t.Parallel()
	...
	if reference.ProtocolID != "aoblmoa-2023-tables5-6" ||
		reference.ClarificationRequest.Artifact !=
			"docs/reference-data/aoblmoa-2023-clarification-request.json" ||
		reference.ClarificationRequest.Status != "awaiting_author_or_archival_data" ||
		reference.ClarificationRequest.TargetAlgorithm != aoblmoaClarificationTarget ||
		len(reference.ClarificationRequest.BlockingQuestionIDs) != 10 ||
		len(reference.ClarificationRequest.ExactPresetQuestionIDs) != 5 {
		t.Fatalf("Tables 5-6 reference does not link the clarification request: %+v", reference)
	}
}
