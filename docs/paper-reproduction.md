...
The completed public-source audit and remaining evidence gate are recorded in
the
[machine-readable AOBLMOA clarification request](reference-data/aoblmoa-2023-clarification-request.json).
Five exact-preset questions cover the article/code strategy, mean, Levy,
population/lifecycle, and evaluation-accounting conflicts. Five separately
gated comparison questions cover the missing classic/CEC drivers, experiment
protocols, ordered seeds and raw trials, statistics, and preserved source
inconsistencies. Its author draft is marked `not_sent`; no request has been
transmitted.
