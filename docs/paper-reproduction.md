...
summary values. They remain descriptive references rather than runnable
reproductions: the paper-linked MATLAB repository supplies only F1 and a
single-run entry point, while seeds, raw runs, the table driver, and the
statistical-analysis driver are absent. The paper and source also disagree on
the Aquila mean and Levy scale, and the current Go default does not yet use the
reported `g=0.9` to `0.4` schedule. The Tables 7-9 artifact preserves and flags
the published Table 8 F10 standard deviation even though it exceeds that row's
reported worst value. The Tables 10-11 artifact also preserves the paper's
rank-sum method and non-average tie ranks; it cannot be regenerated by Mayfly's
paired signed-rank comparison without implementing the paper's method and
recovering all raw samples.
